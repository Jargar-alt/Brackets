...
export function TournamentForm({ onCreate, disabled = false }: Props) {
  ...
  return (
    ...
    >
      ...
      <input className={inputClass} value={name} onChange={(e) => setName(e.target.value)} placeholder="Tournament name" disabled={disabled} />
      <textarea
        ...
        disabled={disabled}
      />
      <select className={inputClass} value={type} onChange={(e) => setType(e.target.value as TournamentType)} disabled={disabled}>
      ...
      <button
        ...
        className="w-full rounded-xl bg-accent px-3 py-2.5 text-sm font-semibold text-white transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-40"
      >
        Create Tournament
      </button>
    </form>
