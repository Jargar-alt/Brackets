...
const inputClass =
  "w-full rounded-xl border border-white/10 bg-surface-raised px-3 py-2 text-sm text-ink placeholder:text-ink-muted transition-colors focus:border-accent/50 focus:outline-none focus:ring-2 focus:ring-accent/20 disabled:opacity-50";
      ...
      >
        <div>
          ...
          <p className="mt-0.5 text-sm text-ink-secondary">
            {participants.length} entered{readOnly ? " · view only" : ""}
          </p>
        </div>
        ...
        <div className="space-y-3 border-t border-white/8 px-4 py-4">
            ...
            <>
              <div className="flex gap-2">
                ...
                <button
                  ...
                  disabled={!name.trim() || nameSet.has(name.trim().toLowerCase())}
                  onClick={async () => {
                    if (!name.trim() || nameSet.has(name.trim().toLowerCase())) return;
          ...
          <div className="max-h-48 space-y-1.5 overflow-y-auto">
            {participants.length === 0 && (
              <p className="py-4 text-center text-sm text-ink-muted">No participants yet.</p>
            )}
